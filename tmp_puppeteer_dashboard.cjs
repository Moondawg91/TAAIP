...
(async () => {
  ...
  try {
    await page.goto('https://taaip.app/dashboard/index.html', {waitUntil: 'domcontentloaded', timeout: 60000});
  } catch (e) {
  ...
  try { await page.screenshot({path:'pm_dashboard_verify.png', fullPage:true}); } catch(e){}

  await browser.close();

  fs.writeFileSync('pm_dashboard_console_verify.json', JSON.stringify(logs, null, 2));
  fs.writeFileSync('pm_dashboard_requests_verify.json', JSON.stringify(requests, null, 2));
  fs.writeFileSync('pm_dashboard_responses_verify.json', JSON.stringify(responses, null, 2));
