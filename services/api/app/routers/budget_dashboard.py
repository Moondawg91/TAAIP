...
def budget_dashboard(fy: int = None, qtr: int = None, org_unit_id: int = None, station_id: str = None, funding_line: str = None) -> Dict[str, Any]:
    conn = connect()
    try:
        cur = conn.cursor()
        ...
        remaining = allocated - planned - actual
        ...
        try:
            ...
            for r in cur.fetchall():
                ...
                by_funding_line.append({'funding_line': fl, 'allocated': a, 'planned': p, 'actual': ac, 'remaining': a - p - ac})
        except Exception:
            by_funding_line = []
        ...
        try:
            ...
            for pr in prows:
                ...
                by_project.append({'project_id': pid, 'name': name, 'planned': planned_p, 'actual': actual_p, 'variance': planned_p - actual_p})
        except Exception:
            by_project = []
        ...
        # Normalize KPIs to the requested shape
        total_planned = planned
        total_spent = actual
        total_pending = max(planned - actual, 0.0)
        total_remaining = allocated - total_spent - total_pending
...
def budget_dashboard_export(fy: int = None, qtr: int = None, org_unit_id: int = None, station_id: str = None, funding_line: str = None):
    # produce a simple CSV with KPI and breakdown rows
    import csv, io
    data = budget_dashboard(fy=fy, qtr=qtr, org_unit_id=org_unit_id, station_id=station_id, funding_line=funding_line)
    buf = io.StringIO()
    ...
    for f in data.get('by_funding_line', []):
