...
router = APIRouter(prefix="/api/command/scoring", tags=["command-scoring"])
...
@router.get('/market-capacity')
def market_capacity(fy: Optional[int] = Query(None), rsid_prefix: Optional[str] = Query(None)):
    conn = connect(); cur = conn.cursor()
    where = '1=1'; params = []
    if fy is not None:
        where += ' AND fy = ?'; params.append(fy)
    if rsid_prefix:
        where += ' AND rsid_prefix LIKE ?'; params.append(f"{rsid_prefix}%")
    try:
        cur.execute(f"SELECT COALESCE(SUM(army_potential),0) as total_army, COALESCE(SUM(potential_remaining),0) as total_remaining FROM market_sama_zip_fact WHERE {where}", params)
        r = cur.fetchone() or {}
        total_army = r.get('total_army') or 0
        total_remaining = r.get('total_remaining') or 0
        # score: capacity score inversely related to potential_remaining / (army+1)
        ratio = _safe_div(total_remaining, (total_army + 1)) or 0
        score = max(0, min(100, int(100 - (ratio * 100))))
        tier = 'HIGH' if score >= 70 else ('MODERATE' if score >= 40 else 'LOW')
        return {'status':'ok','score':score,'tier':tier,'components':{'total_army': int(total_army),'total_remaining': int(total_remaining)},'evidence':{},'missing_data': [], 'data_as_of': _now_iso()}
    except Exception:
        return {'status':'ok','score':0,'tier':'LOW','components':{},'evidence':{},'missing_data':['market_sama_zip_fact missing'],'data_as_of': None}


@router.get('/mission-feasibility')
def mission_feasibility(fy: Optional[int] = Query(None), rsid_prefix: Optional[str] = Query(None)):
    # Simplified deterministic aggregator that uses market_capacity and production facts
    conn = connect(); cur = conn.cursor()
    try:
        cur.execute('SELECT COALESCE(SUM(army_potential),0) as total_army FROM market_sama_zip_fact')
        total_army = (cur.fetchone() or {}).get('total_army') or 0
    except Exception:
        return {'status':'ok','score':0,'tier':'LOW','components':{},'missing_data':['market_sama_zip_fact'],'data_as_of':None}
    # naive production vs requirement: use fact_production if available
    try:
        cur.execute("SELECT COALESCE(SUM(metric_value),0) FROM fact_production WHERE metric_key='accessions' ")
        produced = (cur.fetchone() or [0])[0] or 0
    except Exception:
        produced = 0
    try:
        ratio = _safe_div(produced, total_army) or 0
        score = max(0, min(100, int(ratio * 100)))
        tier = 'HIGH' if score >= 70 else ('MODERATE' if score >= 40 else 'LOW')
        return {'status':'ok','score':score,'tier':tier,'components':{'production': int(produced),'capacity': int(total_army)},'missing_data': [], 'data_as_of': _now_iso()}
    except Exception:
        return {'status':'ok','score':0,'tier':'LOW','components':{},'missing_data': ['fact_production missing'],'data_as_of': None}


@router.get('/resource-alignment')
def resource_alignment():
    # Placeholder deterministic scoring
    return {'status':'ok','score':50,'tier':'MODERATE','components':{},'missing_data': [], 'data_as_of': _now_iso()}
...
@router.get('/cep-effectiveness')
def cep_effectiveness():
    return {'status':'ok','score':50,'tier':'MODERATE','components':{},'missing_data': [], 'data_as_of': _now_iso()}
