...
def market_intel_summary(year: Optional[int] = Query(None), rsid_prefix: Optional[str] = Query(None)):
    ...
    # Prefer canonical mi_* tables when available
    if safe_table_exists(conn, 'mi_zip_fact'):
        try:
            # choose zip column name defensively (zip5)
            zip_col = 'zip5' if _column_exists(conn, 'mi_zip_fact', 'zip5') else ("''")
            if zip_col and zip_col != "''":
                count_expr = f"COUNT(DISTINCT {zip_col})"
            else:
                count_expr = "COUNT(1)"
            q = f"SELECT SUM(COALESCE(fqma,0)) as market_potential_total, SUM(COALESCE(contracts,0)) as contracts_total, SUM(COALESCE(potential_remaining,0)) as potential_remaining_total, SUM(COALESCE(p2p,0)) as p2p_sum, {count_expr} as zip_count FROM mi_zip_fact WHERE 1=1"
            if year:
                q += f" AND fy = {int(year)}"
            if rsid_prefix:
            ...
            cur.execute(q)
            row = cur.fetchone()
            market_potential_total = row[0] or 0
            contracts_total = row[1] or 0
            potential_remaining_total = row[2] or 0
            p2p_sum = row[3] or 0
            zip_count = row[4] or 0
            # army_share_weighted: weighted by fqma if available
            try:
                cur.execute((f"SELECT SUM(COALESCE(army_share,0) * COALESCE(fqma,0)) as wsum, SUM(COALESCE(fqma,0)) as wtot FROM mi_zip_fact WHERE 1=1") + (f" AND fy={int(year)}" if year else "") + (f" AND rsid_prefix='{rsid_prefix}'" if rsid_prefix else ""))
                wrow = cur.fetchone()
                wsum = wrow[0] or 0
                wtot = wrow[1] or 0
                army_share_weighted = (wsum / wtot) if wtot else 0
            except Exception:
                army_share_weighted = 0

            p2p_avg = 0
            try:
                p2p_avg = (p2p_sum / max(1, zip_count))
            except Exception:
                p2p_avg = 0

            contract['kpis'] = {
                'market_potential_total': market_potential_total,
                'contracts_total': contracts_total,
                'potential_remaining_total': potential_remaining_total,
                'army_share_weighted': round(army_share_weighted, 4),
                'p2p_avg': round(p2p_avg, 4),
                'zip_count': zip_count,
                'cbsa_count': 0,
            }
    ...
    # sample series: distribution of markets by cbsa presence
    if safe_table_exists(conn, 'mi_cbsa_fact'):
        try:
            cur.execute("SELECT COUNT(DISTINCT cbsa_code) FROM mi_cbsa_fact")
            cc = cur.fetchone()[0] or 0
            contract['kpis']['cbsa_count'] = cc
            # categories counts
            try:
                cur.execute("SELECT market_category, COUNT(*) as cnt FROM mi_zip_fact GROUP BY market_category")
...
def market_intel_export_targets(fy: Optional[int] = Query(None), qtr: Optional[str] = Query(None), rsid_prefix: Optional[str] = Query(None), component: Optional[str] = Query(None), funding_line: Optional[str] = Query(None), market_category: Optional[str] = Query(None), limit: int = Query(1000)):
    ...
    if safe_table_exists(conn, 'mi_zip_fact'):
        try:
            ...
            # choose columns defensively
            cols = []
            # zip is zip5 or zip
            if _column_exists(conn, 'mi_zip_fact', 'zip5'):
                cols.append('zip5 as zip')
            elif _column_exists(conn, 'mi_zip_fact', 'zip'):
            ...
            if _column_exists(conn, 'mi_zip_fact', 'contracts'):
            ...
            # opportunity_score fallback to p2p
            if _column_exists(conn, 'mi_zip_fact', 'opportunity_score'):
                cols.append('COALESCE(opportunity_score,0) as opportunity_score')
            else:
                cols.append('COALESCE(p2p,0) as opportunity_score')

            q = f"SELECT fy,qtr,rsid_prefix,{', '.join(cols)} FROM mi_zip_fact {where} ORDER BY fy DESC LIMIT {int(limit)}"
            cur.execute(q)
            rows = cur.fetchall()
            lines = [','.join(headers)]
            for r in rows:
    ...
    # Fallback: check market_target_list and attempt to map
    if safe_table_exists(conn, 'market_target_list'):
        try:
            # We will produce minimal columns using available fields
...
def import_templates():
    # Return a small set of import templates metadata for Import Center to consume
    human = [
