...
(async () => {
  ...
  try { await page.screenshot({path:'pm_verify.png', fullPage:true}); } catch(e){}

  await browser.close();

  fs.writeFileSync('pm_console_verify.json', JSON.stringify(logs, null, 2));
  fs.writeFileSync('pm_requests_verify.json', JSON.stringify(requests, null, 2));
  fs.writeFileSync('pm_responses_verify.json', JSON.stringify(responses, null, 2));
