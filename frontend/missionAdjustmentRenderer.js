...
/**
 * Determine the recommended action type based on mission delta percentage.
 * @param {number} deltaPct - The mission delta percentage
 * @returns {string} - 'increase', 'decrease', or 'hold'
 */
function getAdjustmentType(deltaPct) {
  const delta = Number(deltaPct) || 0;
  const threshold = 0.02; // 2% threshold for hold zone
...
/**
 * Get human-readable label and icon for adjustment type.
 * @param {string} adjustmentType - The adjustment type: 'increase', 'decrease', 'hold'
 * @returns {object} - Object with label and icon
 */
function getAdjustmentLabel(adjustmentType) {
  const labels = {
    increase: { label: 'INCREASE mission output', icon: '↑', class: 'maj-increase' },
    decrease: { label: 'DECREASE mission output', icon: '↓', class: 'maj-decrease' },
    hold: { label: 'HOLD mission output at current level', icon: '→', class: 'maj-hold' },
  };
  return labels[adjustmentType] || labels.hold;
}
...
export function buildBriefingModel(payload) {
  ...
  const adjustmentType = getAdjustmentType(mission.delta_pct);
  const adjustmentLabel = getAdjustmentLabel(adjustmentType);
...
function renderList(items, itemRenderer, emptyMessage = EMPTY_TEXT) {
  if (!items.length) {
    return `<li class="maj-empty">${escapeHtml(emptyMessage)}</li>`;
  }
  return items.map(itemRenderer).join('');
}

export function renderMissionAdjustment(payload, container) {
  if (!container) {
    return;
  }
  if (!payload || typeof payload !== 'object') {
    container.innerHTML = '<div class="maj-empty-state">No data returned yet. Submit a request to generate a briefing slide.</div>';
    return;
  }
  ...
  container.innerHTML = `
    <section class="maj-slide" aria-label="Mission adjustment decision slide">
      <header class="maj-header">
        <h1>Mission Adjustment Justification</h1>
        <div class="maj-trace">Request: ${escapeHtml(model.requestId || EMPTY_TEXT)} | Trace: ${escapeHtml(model.traceabilityId || EMPTY_TEXT)}</div>
      </header>

      ${model.isPartialSignals ? '<div class="maj-warning">Partial signals detected. Review evidence and assumptions before final command decision.</div>' : ''}

      <div class="maj-grid">
        <article class="maj-panel maj-top-left">
        ...
        <article class="maj-panel maj-top-right">
          <h2>Recommended Action</h2>
          <div class="maj-action-badge ${model.adjustmentLabel.class}">
            <span class="maj-action-icon">${escapeHtml(model.adjustmentLabel.icon)}</span>
            <span class="maj-action-text">${escapeHtml(model.adjustmentLabel.label)}</span>
          </div>
        ...
        <article class="maj-panel maj-middle-left">
        ...
        <article class="maj-panel maj-middle-right">
        ...
        <article class="maj-panel maj-bottom-left">
        ...
        <article class="maj-panel maj-bottom-right">
      ...
      <section class="maj-panel maj-evidence">
        <h2>Evidence</h2>
        <ul>
          ${renderList(
            model.evidence,
